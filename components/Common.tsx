...
export const Button: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'secondary' | 'danger' | 'ghost' }> = ({ 
  ...
  const variants = {
    ...
    secondary: "bg-surface text-ink-secondary border border-slate-300 hover:bg-slate-50 focus:ring-slate-400",
    danger: "bg-danger text-white hover:bg-red-600 focus:ring-danger",
    ghost: "text-ink-secondary hover:bg-slate-100 focus:ring-slate-400",
  };
...
export const Card: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ children, className = '', ...props }) => (
  <div className={`bg-surface rounded-card shadow-sm border border-slate-200 ${className}`} {...props}>
    {children}
  </div>
);
