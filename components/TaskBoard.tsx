...
import { Button, PillSegmentedControl, LoadBar } from './Common';
...
const TaskBoard: React.FC<TaskBoardProps> = ({ tasks, users, currentUser, onAddTask, onUpdateTask, onBatchUpdateTasks, onDeleteTask, onEscalateTask, escalations = [], onResolveEscalation, onCloseEscalation }) => {
  ...
  const filteredTasks = tasks.filter(t => {
    ...
    if (filter === 'MY_TASKS' && !t.assigneeIds.includes(currentUser.id)) return false;
    if (searchQuery.trim() && !t.title.toLowerCase().includes(searchQuery.trim().toLowerCase())) return false;
    return true;
  });

  const columns = [
    { id: TaskStatus.TODO, label: 'To Do', bg: 'bg-[#eceef2]', dotColor: 'bg-ink-muted', labelColor: 'text-ink-secondary', countBg: 'bg-[#dde1ea] text-ink-secondary' },
    { id: TaskStatus.IN_PROGRESS, label: 'In Progress', bg: 'bg-[#eaeffb]', dotColor: 'bg-accent', labelColor: 'text-[#3d5fbf]', countBg: 'bg-[#dbe4fb] text-[#3d5fbf]' },
    { id: TaskStatus.ON_HOLD, label: 'On Hold', bg: 'bg-warning-bg', dotColor: 'bg-warning', labelColor: 'text-warning-text', countBg: 'bg-[#f5e3c2] text-warning-text' },
    { id: TaskStatus.DONE, label: 'Complete', bg: 'bg-[#e7f5ee]', dotColor: 'bg-success', labelColor: 'text-success-text', countBg: 'bg-success-bg text-success-text' },
  ];

  // Team Load: active (non-done, non-hidden) task count per agent, relative to the busiest agent.
  const agents = users.filter(u => u.role === UserRole.AGENT);
  const loadCounts = useMemo(() => {
    const counts: Record<string, number> = {};
    agents.forEach(a => { counts[a.id] = 0; });
    tasks.forEach(t => {
      if (t.status === TaskStatus.DONE || t.hiddenFromBoard) return;
      t.assigneeIds.forEach(id => { if (counts[id] !== undefined) counts[id]++; });
    });
    return counts;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tasks, users]);
  const maxLoad = Math.max(1, ...Object.values<number>(loadCounts));

  const getTypeStyles = (t: TaskType) => {
    switch (t) {
      case TaskType.AUGMENTING: return 'bg-purple-bg text-purple';
      case TaskType.QA: return 'bg-danger-bg text-danger-text';
      case TaskType.INVOICE_PROCESSING: return 'bg-blue-50 text-blue-700';
      case TaskType.CHECK_404: return 'bg-success-bg text-success-text';
      case TaskType.DATA_REFRESHER: return 'bg-success-bg text-success-text';
      case TaskType.OTHERS: return 'bg-slate-100 text-ink-secondary';
  ...
  const openCount = filteredTasks.filter(t => t.status !== TaskStatus.DONE).length;
  const escalatedCount = filteredTasks.filter(t => t.isEscalated).length;
  ...
  return (
    <div className="h-full flex flex-col gap-4">
      ...
      <div className="flex justify-between items-end">
        <div>
           <h2 className="text-[21px] font-bold text-ink tracking-tight">Task Board</h2>
           <p className="text-xs text-ink-muted mt-0.5">{openCount} open{escalatedCount > 0 ? ` · ${escalatedCount} escalated` : ''}</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-[200px] h-8 rounded-lg bg-surface border border-slate-200 px-3 text-[11.5px] text-ink placeholder:text-ink-muted focus:outline-none focus:ring-2 focus:ring-accent"
          />
          <PillSegmentedControl
            value={filter}
            onChange={v => setFilter(v as 'ALL' | 'MY_TASKS')}
            options={[{ value: 'ALL', label: 'All' }, { value: 'MY_TASKS', label: 'Mine' }]}
          />

          {currentUser.role === UserRole.MANAGER && (
            <button
              onClick={() => setShowArchiveConfirm(true)}
              className="px-3.5 py-2 text-[11.5px] font-semibold rounded-lg bg-surface text-ink-secondary hover:bg-slate-50 transition-colors border border-slate-200"
            >
              Archive
            </button>
          )}
          ...
          {currentUser.role === UserRole.MANAGER && (
            <Button onClick={() => setIsCreateModalOpen(true)} className="text-[12px] shadow-md shadow-blue-500/20">
                + New Task
            </Button>
          )}
        </div>
      ...
      {/* Team Load */}
      {agents.length > 0 && (
        <div className="bg-surface rounded-card px-4 py-3 shadow-sm flex items-center gap-5">
          <div className="text-[11px] font-bold text-ink-muted uppercase tracking-wide flex-shrink-0">Team Load</div>
          <div className="flex gap-6 flex-1 flex-wrap">
            {agents.map(a => (
              <div key={a.id} className="w-40">
                <LoadBar name={a.name} avatar={a.avatar} count={loadCounts[a.id] || 0} maxCount={maxLoad} />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 overflow-x-auto">
        <div className="flex gap-3.5 min-w-full h-full pb-4">
          {columns.map(col => {
            const columnTasks = filteredTasks.filter(t => t.status === col.id);

            return (
              <div key={col.id} className={`w-80 flex flex-col h-full rounded-card ${col.bg} p-3 flex-shrink-0`}>
                {/* Column Header */}
                <div className="flex justify-between items-center px-1 pb-2.5">
                  <div className={`flex items-center gap-1.5 text-[11.5px] font-bold uppercase tracking-wide ${col.labelColor}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${col.dotColor}`}></span>
                    {col.label}
                  </div>
                  <span className={`text-[11px] font-bold px-1.5 py-0.5 rounded-full ${col.countBg}`}>
                    {columnTasks.length}
                  </span>
                </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar">
                  {columnTasks.map(task => {
                    ...
                    const borderColorClass = task.status === TaskStatus.DONE ? 'border-l-success'
                      : task.priority === TaskPriority.CRITICAL ? 'border-l-danger'
                      : task.priority === TaskPriority.HIGH ? 'border-l-warning'
                      : task.priority === TaskPriority.MEDIUM ? 'border-l-accent'
                      : 'border-l-slate-400';

                    return (
                      <div
                        key={task.id}
                        className={`bg-surface rounded-lg pl-3.5 pr-3 py-3 shadow-sm hover:shadow-md transition-all cursor-pointer group border-l-[3px] ${borderColorClass} ${task.status === TaskStatus.DONE ? 'opacity-70' : ''}`}
                        onClick={() => handleTaskClick(task)}
                      >
                        {/* Tags */}
                        <div className="flex justify-between items-start mb-2">
                           <span className={`px-1.5 py-0.5 rounded text-[9.5px] font-bold tracking-wide uppercase ${getTypeStyles(task.type)}`}>
                             {task.type === TaskType.OTHERS && task.customType ? task.customType : getTypeLabel(task.type)}
                           </span>
                           <div className="flex items-center gap-1.5">
                               {task.isEscalated && <Flag className="w-3 h-3 text-danger" title="Escalated" />}
                               {currentUser.role === UserRole.MANAGER && (
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
                                        ...
                                        className="text-ink-muted hover:text-danger transition-colors opacity-0 group-hover:opacity-100"
                                        title="Delete Task"
                                    >
                        ...
                        {/* Title */}
                        <h4 className={`text-[13px] font-semibold text-ink leading-snug ${task.status === TaskStatus.DONE ? 'line-through decoration-slate-300' : ''}`}>
                            {task.title}
                        </h4>
                        ...
                        <div className="flex items-center justify-between mt-3">
                          <div className="flex -space-x-1.5">
                            {assignees.slice(0, 3).map(assignee => (
                              assignee.avatar ? (
                                <img key={assignee.id} src={assignee.avatar} alt={assignee.name} className="w-[19px] h-[19px] rounded-full border border-white" title={assignee.name} />
                              ) : (
                                <div key={assignee.id} className="w-[19px] h-[19px] rounded-full bg-slate-100 border border-white flex items-center justify-center text-[9px] font-bold text-ink-secondary" title={assignee.name}>
                                  {assignee.name.charAt(0)}
                                </div>
                              )
                            ))}
                            {assignees.length === 0 && (
                              <span className="text-[10.5px] text-ink-muted">Unassigned</span>
                            )}
                          </div>

                          <div className="flex items-center gap-2">
                             {task.attachments && task.attachments.length > 0 && (
                                <span className="text-[10.5px] text-ink-muted" title={`${task.attachments.length} attachments`}>📎 {task.attachments.length}</span>
                             )}
                             <span className={`text-[11px] font-semibold ${isOverdue ? 'text-danger-text' : 'text-ink-muted'}`}>
                  ...
                  {columnTasks.length === 0 && (
                    <div className="h-24 flex items-center justify-center border-2 border-dashed border-slate-200/80 rounded-xl">
                        <span className="text-ink-muted text-xs font-medium">No tasks</span>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[60]">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 animate-fadeIn">
            ...
            <p className="text-slate-600 mb-6">
              ...
              They'll be hidden from this board immediately - the tasks aren't deleted, just hidden.
            </p>
