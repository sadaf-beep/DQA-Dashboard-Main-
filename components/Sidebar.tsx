...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  menuItems.push({ id: 'leave-holidays', label: 'Leave & Holidays', icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' });
    ...
    <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col h-full fixed left-0 top-0 z-50">
      ...
      <div className="p-4 border-t border-slate-800">
        {actualUserRole === UserRole.MANAGER && (
          <div className="mb-4">
            <button 
              onClick={onToggleViewAsAgent}
              className={`w-full flex items-center justify-between px-3 py-2 text-sm font-medium rounded-lg transition-colors border ${
                isViewingAsAgent 
            ...
            >
              ...
              <div className={`w-8 h-4 rounded-full relative transition-colors ${isViewingAsAgent ? 'bg-blue-500' : 'bg-slate-600'}`}>
                <div className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white transition-transform ${isViewingAsAgent ? 'translate-x-4' : ''}`}></div>
              </div>
            </button>
          </div>
        )}
